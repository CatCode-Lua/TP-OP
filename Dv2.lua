local function safeErr(fn)
    local ok, err = xpcall(fn, function(e)
        warn("[ZAY ERROR]", e, debug.traceback())
    end)
    return ok
end
...
local function teleportDown()
    ...
    local newPos = rootPart.Position + Vector3.new(0, -Values.TPDownDistance, 0)
...
local ok2, err = pcall(function()
    -- pega aquí solo el bloque del main Frame y titleTxt
end)
if not ok2 then warn("UI ERROR:", err) end
...
titleTxt.BackgroundTransparency=1; titleTxt.Text="ANTILOSER HELPER V2 + TP DOWN"; titleTxt.TextColor3=C.white
...
createSectionHeader(o,"TP DOWN") o+=1
createSlider(o, "TP Down Distance", 1, 100, "TPDownDistance", function(v)
    Values.TPDownDistance = v
end) o+=1

createButtonRow(o, "⬇️  TP DOWN", Color3.fromRGB(28,28,28), C.dim, function(btn)
    teleportDown()
    btn.Text = "⬇️  DOWN!"
    ...
    playSound("rbxassetid://6895079813", 0.3, 1.2)
end) o+=1
...
createButtonRow(o,"💾  SAVE CONFIG",Color3.fromRGB(28,28,28),C.dim,
    function(btn)
        ...
        playSound("rbxassetid://6895079813",0.3,1)
    end) o+=1
...
if Enabled.Aimbot and not isMobile then abToggle() end

UserInputService.InputBegan:Connect(function(inp, gpe)
    if gpe then return end
    for k, key in pairs(KEYBINDS) do
        if inp.KeyCode == key then
            if k == "SPEED" then
                local newState = not Enabled.SpeedBoost
                Enabled.SpeedBoost = newState
                if VisualSetters.SpeedBoost then VisualSetters.SpeedBoost(newState, true) end
                if newState then startSpeedBoost() else stopSpeedBoost() end
            elseif k == "SPIN" then
                local newState = not Enabled.SpinBot
                Enabled.SpinBot = newState
                if VisualSetters.SpinBot then VisualSetters.SpinBot(newState, true) end
                if newState then startSpinBot() else stopSpinBot() end
            elseif k == "GALAXY" then
                local newState = not Enabled.Galaxy
                Enabled.Galaxy = newState
                if VisualSetters.Galaxy then VisualSetters.Galaxy(newState, true) end
                if newState then startGalaxy() else stopGalaxy() end
            elseif k == "AUTOLEFT" then
            ...
            elseif k == "ANTIRAGDOLL" then
                local newState = not Enabled.AntiRagdoll
                Enabled.AntiRagdoll = newState
                if VisualSetters.AntiRagdoll then VisualSetters.AntiRagdoll(newState, true) end
                if newState then startAntiRagdoll() else stopAntiRagdoll() end
            elseif k == "AIMBOT" and not isMobile then
                abToggle()
                if VisualSetters.Aimbot then VisualSetters.Aimbot(abActive, true) end
            elseif k == "FLOAT" then
                local newState = not Enabled.Float
                Enabled.Float = newState
                if VisualSetters.Float then VisualSetters.Float(newState, true) end
                if newState then startFloat() else stopFloat() end
            elseif k == "FULLAUTODUEL" then
                local newState = not Enabled.FullAutoDuel
                Enabled.FullAutoDuel = newState
                if VisualSetters.FullAutoDuel then VisualSetters.FullAutoDuel(newState, true) end
                if newState then startFullAutoDuel() else stopFullAutoDuel() end
            end
        end
    end
end)
...
print("Script cargado con TP Down incluido.")
]])()
